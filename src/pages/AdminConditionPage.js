...
const AdminConditionPage = () => {
  ...
  const [currentCondicion, setCurrentCondicion] = useState(null);
  const [newCondicion, setNewCondicion] = useState({ nombre: '' });
  ...
  const handleCreateCondicion = async (e) => {
    e.preventDefault();
    const token = getAuthToken();
    if (!token) return;
    ...
    try {
      const response = await fetch(`${API_URL}/crear_condicion/`, {
        ...
        body: JSON.stringify(newCondicion)
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Error al crear condición');
      }

      setSuccess('Condición creada exitosamente');
      setOpenCreateModal(false);
      setNewCondicion({ nombre: '' });
      fetchCondiciones();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const handleDeleteCondicion = async () => {
    ...
    try {
      ...
      setSuccess('Condición eliminada exitosamente');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Administración de Condiciones</h1>
              <p className="text-gray-600">Gestiona las condiciones de los productos</p>
            </div>
            <button 
        ...
        {loading.list ? (
          <div className="bg-white rounded-2xl shadow-lg p-16">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-gray-600 font-medium">Cargando condiciones...</p>
            </div>
          </div>
        ) : error ? (
              ...
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {condiciones.map((condicion, index) => (
                    <tr key={condicion.id} className={`hover:bg-orange-50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">#{condicion.id}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0">
                            <div className="h-10 w-10 rounded-lg bg-orange-100 flex items-center justify-center">
                              <GradeIcon className="text-orange-600" sx={{ fontSize: 20 }} />
                            </div>
                          </div>
                          <div>
                            <div className="text-sm font-medium text-gray-900">{condicion.nombre}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
            ...
            {condiciones.length === 0 && (
              <div className="text-center py-16">
                <div className="text-gray-400 mb-4">
                  <GradeIcon sx={{ fontSize: 64 }} />
                </div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">No hay condiciones configuradas</h3>
                <p className="text-gray-500 mb-6">Configura tu primera condición para comenzar</p>
                <button 
                  onClick={() => setOpenCreateModal(true)}
        ...
        >
          <DialogTitle className="text-center pb-2">
            <h2 className="text-2xl font-bold text-gray-900">Crear Nueva Condición</h2>
            <p className="text-gray-500 text-sm mt-1">Define una nueva condición para los productos</p>
          </DialogTitle>
          
          <form onSubmit={handleCreateCondicion}>
            <DialogContent className="space-y-4">
              <TextField
                label="Nombre de la Condición"
                variant="outlined"
                fullWidth
                value={newCondicion.nombre}
                onChange={(e) => setNewCondicion({ ...newCondicion, nombre: e.target.value })}
                required
                margin="normal"
                placeholder="Ej: Grado A, Nuevo, Usado..."
                sx={{
        ...
        >
          <DialogTitle className="text-center pb-2">
            <h2 className="text-2xl font-bold text-gray-900">Editar Condición</h2>
            <p className="text-gray-500 text-sm mt-1">Actualiza la información de la condición</p>
          </DialogTitle>
            ...
            {currentCondicion && (
              <div className="space-y-4">
                ...
                <p className="text-gray-700">
                  ¿Estás seguro de que deseas eliminar la condición <strong>{currentCondicion.nombre}</strong>?
                </p>
              </div>
            )}
          </DialogContent>
...
export default AdminConditionPage;