...
const AdminDivisaPage = () => {
  ...
  const [newMoneda, setNewMoneda] = useState({ nombre: '', cambio: '' });
  ...
  // Obtener detalles de una moneda
  const fetchMonedaDetails = async (id) => {
    const token = getAuthToken();
    if (!token) return;
  ...
  // Crear nueva moneda
  const handleCreateMoneda = async (e) => {
    e.preventDefault();
    const token = getAuthToken();
    if (!token) return;
  ...
  const handleEditMoneda = async (e) => {
    e.preventDefault();
    const token = getAuthToken();
    ...
    try {
      const response = await fetch(`${API_URL}/editar_moneda/${currentMoneda.id}/`, {
        ...
        body: JSON.stringify({
          nombre: currentMoneda.nombre,
          cambio: currentMoneda.cambio
        })
      });
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Error al editar moneda');
      }
  ...
  // Eliminar moneda
  const handleDeleteMoneda = async () => {
    const token = getAuthToken();
    if (!token || !currentMoneda) return;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Error al eliminar moneda');
      }

      setSuccess('Moneda eliminada exitosamente');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Administración de Monedas</h1>
              <p className="text-gray-600">Gestiona las divisas y tipos de cambio</p>
            </div>
            <button 
        ...
        {loading.list ? (
          <div className="bg-white rounded-2xl shadow-lg p-16">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-gray-600 font-medium">Cargando monedas...</p>
            </div>
          </div>
        ) : error ? (
              ...
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {monedas.map((moneda, index) => (
                    <tr key={moneda.id} className={`hover:bg-orange-50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">#{moneda.id}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0">
                            <div className="h-10 w-10 rounded-lg bg-orange-100 flex items-center justify-center">
                              <AccountBalanceWalletIcon className="text-orange-600" sx={{ fontSize: 20 }} />
                            </div>
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-semibold text-green-600 bg-green-50 px-3 py-1 rounded-full inline-block">
                          {formatCambio(moneda.cambio)}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center space-x-3">
                          <button
                            onClick={() => fetchMonedaDetails(moneda.id)}
            ...
            {monedas.length === 0 && (
              <div className="text-center py-16">
                <div className="text-gray-400 mb-4">
                  <AccountBalanceWalletIcon sx={{ fontSize: 64 }} />
                </div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">No hay monedas configuradas</h3>
                <p className="text-gray-500 mb-6">Configura tu primera moneda para comenzar</p>
                <button 
                  onClick={() => setOpenCreateModal(true)}
        ...
        >
          <DialogTitle className="text-center pb-2">
            <h2 className="text-2xl font-bold text-gray-900">Crear Nueva Moneda</h2>
            <p className="text-gray-500 text-sm mt-1">Configura una nueva divisa y su tipo de cambio</p>
          </DialogTitle>
          
          <form onSubmit={handleCreateMoneda}>
            <DialogContent className="space-y-4">
              ...
              <TextField
                ...
                type="number"
                fullWidth
                value={newMoneda.cambio}
                onChange={(e) => setNewMoneda({ ...newMoneda, cambio: e.target.value })}
                required
                margin="normal"
        ...
        >
          <DialogTitle className="text-center pb-2">
            <h2 className="text-2xl font-bold text-gray-900">Editar Moneda</h2>
            <p className="text-gray-500 text-sm mt-1">Actualiza la información de la moneda</p>
          </DialogTitle>
          
          <form onSubmit={handleEditMoneda}>
            <DialogContent className="space-y-4">
              {currentMoneda && (
                <>
                  <TextField
                  ...
                  <TextField
                    ...
                    type="number"
                    fullWidth
                    value={currentMoneda.cambio}
                    onChange={(e) => setCurrentMoneda({ ...currentMoneda, cambio: e.target.value })}
                    required
                    margin="normal"
                    inputProps={{ step: "0.01" }}
                    sx={{
            ...
            {currentMoneda && (
              <div className="space-y-4">
                ...
                <p className="text-gray-700">
                  ¿Estás seguro de que deseas eliminar la moneda <strong>{currentMoneda.nombre}</strong>?
                </p>
                <div className="bg-red-50 border border-red-200 rounded-xl p-3">
                  <p className="text-red-700 text-sm">
                    Tipo de cambio actual: {formatCambio(currentMoneda.cambio)}
                  </p>
                </div>
              </div>
            )}
...
export default AdminDivisaPage;